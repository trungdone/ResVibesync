...
import { fetchArtistById } from "../artists/artistsApi";
import { fetchAlbums } from "../albums/albumsApi";
  ...
  useEffect(() => {
    async function loadArtistsAndAlbums() {
      try {
        ...
        const albumMap = {}; // songId -> [albumTitle]
        for (const album of albums) {
          for (const songId of album.songs || []) {
            if (!albumMap[songId]) {
              albumMap[songId] = [];
            }
            albumMap[songId].push(album.title);
          }
        }
